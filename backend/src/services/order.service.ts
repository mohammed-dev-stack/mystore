...
import Order, { IOrder, IOrderItem, IAddress } from '../models/order.model.js';
import Product from '../models/product.model.js';
import { AppError } from '../middleware/error.middleware.js';
import logger from '../utils/logger.js';

// Type for payment result
...
type OrderStatus = 'pending' | 'payment_failed' | 'paid' | 'processing' | 'shipped' | 'delivered' | 'cancelled' | 'refunded';
type Carrier = 'fedex' | 'ups' | 'usps' | 'dhl' | 'other' | null;

// Mock payment gateway (replace with Stripe/PayPal SDK in production)
const processPayment = async (amount: number, currency: string, paymentMethodId: string): Promise<PaymentResult> => {
...
export const getOrderById = async (orderId: string, userId: string, userRole: string) => {
  ...
  const order = await Order.findById(orderId)
    .populate<{ user: { _id: mongoose.Types.ObjectId; fullName: string; email: string } }>('user', 'fullName email')
    .lean();
  if (!order) {
...
export const updateOrderStatus = async (
  orderId: string,
  status: OrderStatus,
  shippingInfo?: { trackingNumber?: string; carrier?: Carrier; estimatedDelivery?: Date }
) => {
  ...
  if (status === 'shipped' && shippingInfo) {
    ...
    if (shippingInfo.carrier) order.carrier = shippingInfo.carrier;
    if (shippingInfo.estimatedDelivery) order.estimatedDelivery = shippingInfo.estimatedDelivery;
  }
...
export const handlePaymentWebhook = async (rawBody: string, signature: string) => {
