...
const generateUniqueSlug = async (baseName: string, excludeId?: string): Promise<string> => {
  ...
  const excludeIdStr = excludeId as string | undefined;
...
const buildFilter = (options: ProductFilterOptions): any => {
  const filter: any = {};
...
export const updateProduct = async (id: string, updateData: Partial<IProduct>, updatedBy: string) => {
  if (!mongoose.Types.ObjectId.isValid(id)) {
  ...
  for (const field of allowedUpdates) {
    if (updateData[field] !== undefined) {
      (product as unknown as Record<string, unknown>)[field] = updateData[field];
    }
  }
...
export const getProductsByCategory = async (category: string, page = 1, limit = 20) => {
  const skip = (page - 1) * limit;
  const [products, total] = await Promise.all([
...
export const searchProducts = async (keyword: string, limit = 20) => {
...
export const smartSearchProducts = async (query: string, limit = 5): Promise<any[]> => {
  ...
  if (results.length) {
    const scored = results.map(p => {
      ...
      words.forEach(word => {
        ...
        if (re.test(p.description)) score += 1;
        if (p.tags?.some((tag: string) => re.test(tag))) score += 2;
      });
...
export const getFallbackProducts = async (limit = 5): Promise<any[]> => {
 ...
 */
export const autocompleteProducts = async (prefix: string, limit: number = 5): Promise<any[]> => {
