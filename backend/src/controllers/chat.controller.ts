 ...
 */
export const getConversationHistory = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  if (!req.user || !req.user._id) {
    res.status(401).json({ success: false, message: 'User not authenticated' });
    return;
  }
 ...
 */
export const getUserSessions = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  if (!req.user || !req.user._id) {
    res.status(401).json({ success: false, message: 'User not authenticated' });
    return;
  }
 ...
 */
export const deleteSession = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  if (!req.user || !req.user._id) {
    res.status(401).json({ success: false, message: 'User not authenticated' });
    return;
  }
 ...
 */
export const clearAllHistory = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  if (!req.user || !req.user._id) {
    res.status(401).json({ success: false, message: 'User not authenticated' });
    return;
  }
