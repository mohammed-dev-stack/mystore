 ...
 */
export const textSearch = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { q, limit = 20 } = req.query;
  if (!q || (q as string).trim().length === 0) {
    res.status(400).json({ success: false, message: 'Search query is required' });
    return;
  }
  const results = await productService.searchProducts(q as string, parseInt(limit as string));
  res.status(200).json({
    success: true,
 ...
 */
export const imageSearch = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const combinedSearch = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const autocomplete = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const suggestions = await productService.autocompleteProducts(q as string, parseInt(limit as string));
