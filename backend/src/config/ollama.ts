...
export const checkOllamaHealth = async (): Promise<HealthStatus> => {
  try {
    const response = await ollamaClient.get<OllamaTagResponse>('/api/tags', { timeout: 5000 });
 ...
 */
export const analyzeImageWithOllama = async (imageBase64: string): Promise<string> => {
  try {
    ...
    const response = await ollamaClient.post<OllamaGenerateResponse>('/api/generate', payload);
...
export const chatWithOllama = async (
  ...
  try {
    ...
    const response = await ollamaClient.post<OllamaChatResponse>('/api/chat', payload);
...
// ============================
// Batch Image Analysis (max 5 images)
// ============================
 ...
 */
export const batchAnalyzeImages = async (imageBase64Array: string[]): Promise<BatchAnalysisResult[]> => {
